...
export function decrypt_con_notify_data(encrypted_b64: string): string {
    ...
    const data = Uint8Array.from(atob(encrypted_b64), c => c.charCodeAt(0));
    ...
    const nonce = data.slice(-28, -16);
    const ciphertext = data.slice(0, -28);

    // Use Web Crypto API for AES-GCM
    // For now, throw error as crypto-js doesn't support GCM easily
    throw new Error("AES-GCM decryption not implemented. Use Web Crypto API.");
}
...
export async function make_remote_request(path: string, body: any, token: string, method: string = "GET"): Promise<any> {
...
export async function make_local_request(path: string, body?: string, headers?: Record<string, string>): Promise<Response | null> {
    try {
...
export async function send_sdp_to_remote_peer(serial: string, sdp: string, access_token: string, public_key: forge.pki.rsa.PublicKey): Promise<string> {
