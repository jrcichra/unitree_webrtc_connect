...
class FutureResolver {
    ...
    saveResolve(messageType: string, topic: string, resolver: { resolve: (value: any) => void; reject: (reason?: any) => void }, identifier: string) {
    ...
    runResolveForTopic(message: any) {
        ...
        if (message.type === DATA_CHANNEL_TYPE.RTC_INNER_REQ &&
    ...
    private runResolveForTopicForFile(message: any) {
        ...
        if (fileInfo && fileInfo.enable_chunking) {
            ...
            if (this.chunkDataStorage[key]) {
                this.chunkDataStorage[key].push(typeof dataChunk === 'string' ? new TextEncoder().encode(dataChunk) : dataChunk);
            } else {
                this.chunkDataStorage[key] = [typeof dataChunk === 'string' ? new TextEncoder().encode(dataChunk) : dataChunk];
            }
...
class WebRTCDataChannelPubSub {
    ...
    async publish(topic: string, data: any = null, msgType: string = DATA_CHANNEL_TYPE.MSG): Promise<any> {
        return new Promise((resolve, reject) => {
            if (this.channel.readyState === "open") {
                ...
                const uuid = get_nested_field(data, "uuid") ||
    ...
    async publishRequestNew(topic: string, options: any = {}): Promise<any> {
        ...
        if (options && options.parameter) {
            requestPayload.parameter = typeof options.parameter === 'string' ? options.parameter : JSON.stringify(options.parameter);
        }
...
class WebRTCDataChannelFileUploader {
    ...
    async uploadFile(data: Uint8Array, filePath: string, chunkSize: number = 60 * 1024, progressCallback?: (progress: number) => void): Promise<string> {
        // Encode to base64
...
class WebRTCDataChannelFileDownloader {
    ...
    async downloadFile(filePath: string, chunkSize: number = 60 * 1024, progressCallback?: (progress: number) => void): Promise<Uint8Array> {
